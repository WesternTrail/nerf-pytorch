...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    outputs_flat = batchify(fn, netchunk)(embedded) # # return:（65536,4）
...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    Args:
      ...
      chunk: int. Maximum number of rays to process simultaneously. Used to 同步处理的最大光线数
        control maximum memory usage. Does not affect final results.
      rays: array of shape [2, batch_size, 3]. Ray origin and direction for
        each example in batch. 表示每个batch的原点和方向；
    ...
    if use_viewdirs:
        ...
        viewdirs = viewdirs / torch.norm(viewdirs, dim=-1, keepdim=True) # 视图方向进行了标准化
    ...
    # 生成光线的远近端，用于确定边界框，将其聚合到rays中
    near, far = near * torch.ones_like(rays_d[...,:1]), far * torch.ones_like(rays_d[...,:1])
    rays = torch.cat([rays_o, rays_d, near, far], -1) # 3+3+2
    if use_viewdirs:
        rays = torch.cat([rays, viewdirs], -1)

    # 开始并行计算光线属性
    all_ret = batchify_rays(rays, chunk, **kwargs) # rgb0代表的是微调之后的
    for k in all_ret:
...
def create_nerf(args):
    ...
    model = NeRF(D=args.netdepth, W=args.netwidth,
                 ...
                 input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device) # 定义粗的网络
    grad_vars = list(model.parameters())

    model_fine = None
    if args.N_importance > 0: # 每条射线附加的细样本数
        model_fine = NeRF(D=args.netdepth_fine, W=args.netwidth_fine,
                          input_ch=input_ch, output_ch=output_ch, skips=skips,
                          input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device)
        grad_vars += list(model_fine.parameters()) # 梯度加上了精修的网络梯度
    ...
    render_kwargs_train = {
        ...
        'network_fine' : model_fine, # 论文提到的精修网络
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    # weights = alpha * tf.math.cumprod(1.-alpha + 1e-10, -1, exclusive=True) 1-alpha就是ti
    weights = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)), 1.-alpha + 1e-10], -1), -1)[:, :-1]
    rgb_map = torch.sum(weights[...,None] * rgb, -2) # 注意是对所有点求和  # [N_rays, 3]
...
def render_rays(ray_batch,
    ...
    viewdirs = ray_batch[:,-3:] if ray_batch.shape[-1] > 8 else None # 这个跟rays_d的区别就是经过了标准化
    ...
    z_vals = z_vals.expand([N_rays, N_samples]) # 没啥作用
    ...
    # 每个采样点的3D坐标，o+td
    pts = rays_o[...,None,:] + rays_d[...,None,:] * z_vals[...,:,None] # 1024条光线，每条光线上选了64个点 # [N_rays, N_samples, 3]


#     raw = run_network(pts)
    raw = network_query_fn(pts, viewdirs, network_fn)
    # 体素渲染！将光束上采样的点颜色合成图像上的点
    ...
    if N_importance > 0:
        ...
        z_samples = sample_pdf(z_vals_mid, weights[...,1:-1], N_importance, det=(perturb==0.), pytest=pytest) # 根据粗网络渲染的结果得到的weight进行采样
...
def config_parser():
    ...
    # batch size指的是每次梯度下降的随机射线的数量,默认是4096
    ...
    # 渲染测试集而不是用render_poses
    parser.add_argument("--render_test", action='store_true',
                        help='render the test set instead of render_poses path')
    # 下采样因子以加快渲染速度，设置为4或者8用于快速预览
    ...
    # 从测试/验证集中加载1/N图像，这对于像 deepvoxels 这样的大型数据集有用
...
def train():
    ...
    if args.dataset_type == 'llff':
        # shape: images[图片数,H,W,channel(3)] poses[图片数,3,5] render_poses[120,3,5]
        images, poses, bds, render_poses, i_test = load_llff_data(args.datadir, args.factor,
                                                                  recenter=True, bd_factor=.75,
                                                                  spherify=args.spherify)
        hwf = poses[0,:3,-1] # 3x5矩阵的最后一列存储了相机的内参，长,宽,焦距，代表了相机的内参
        poses = poses[:,:3,:4] # 位姿外参3x4矩阵？取出平移和旋转部分
    ...
    render_kwargs_train.update(bds_dict) # 加入三维空间的立方体
    ...
    N_rand = args.N_rand # 在图片中随机抽1024个点？
    use_batching = not args.no_batching
    if use_batching: # 当图片很少时采用这个方法
        # For random ray batching
        print('get rays')
        # 对所有图片的每个像素生成光线。P：外参矩阵。ro:代表相机(光线)的起点，rd代表光线的方向向量
        rays = np.stack([get_rays_np(H, W, K, p) for p in poses[:,:3,:4]], 0) # [N, ro+rd, H, W, 3]
        print('done, concats')
        rays_rgb = np.concatenate([rays, images[:,None]], 1) # 光线带上了当前像素的颜色[N, ro+rd+rgb, H, W, 3]
    ...
    for i in trange(start, N_iters):
        ...
        trans = extras['raw'][...,-1]
        loss = img_loss
        psnr = mse2psnr(img_loss) # 峰值信噪比

        if 'rgb0' in extras: # 加上精修的损失
        ...
        if i%args.i_video==0 and i > 0:  # 训练到第5万次迭代，开始生成视频
